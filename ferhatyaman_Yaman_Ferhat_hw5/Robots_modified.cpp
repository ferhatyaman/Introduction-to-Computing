...
void Robot::SetBagCount(int num)
{
	bag = num;
	theRobotWindow->Redraw(this);
}
...
{
	for (int a = GetCellCount(xPos, yPos); a != 0; a--)
	{
		PickThing();
	}
	theRobotWindow->Redraw(this);
}
bool Robot::Resurrect()
{
	if (stalled)
		return !stalled;
	theRobotWindow->Redraw(this);
...
bool PutThingInCell(int x, int y,unsigned long difference, int things){
	difference/=CLOCKS_PER_SEC;
	if(difference >= 1){
		theRobotWorld->IncrCount(x, y, things);
